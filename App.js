...
export default () => {
  ...
  const [topEdge, setTopEdge] = React.useState(null);
  const scrollY = React.useRef(new Animated.Value(0)).current;

  React.useEffect(() => {
    if (bottomActions) {
      setTopEdge(bottomActions.y - height + bottomActions.height);
      console.log('📢 [App.js:28]', bottomActions.y - height + bottomActions.height);
    }

  }, [JSON.stringify(bottomActions)])
        ...
        <View>
          <Text style={styles.featuredTitle}>Featured</Text>
          {
            articleParagraphs.slice(0, 3).map((text, index) => {
              return (
                <View key={`featured-${index}`} style={{ flexDirection: 'row', marginBottom: 80 }}>
